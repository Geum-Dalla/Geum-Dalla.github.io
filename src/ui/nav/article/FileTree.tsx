...
import { FileNode, FolderNode, isFolderNode, isFileNode } from "@/lib/articles/types";
...
export default function FileTree({ isActive, Tree }: FileTreeProps) {
  const [selectedByFolderId, setSelectedByFolderId] = useState<Record<string, string>>({});
  ...
  const columns = useMemo(() => {
    ...
    while (current) {
      ...
      const next = current.children.find((n) => isFolderNode(n) && n.id === selectedChildId) as FolderNode | undefined;
    ...
    return cols;
  }, [Tree, selectedByFolderId]);

  useEffect(() => {
    setEnterIndex(columns.length - 1);
    const t = setTimeout(() => setEnterIndex(null), 20);
    return () => clearTimeout(t);
  }, [columns.length]);

  const handleFolderClick = (parentFolder: FolderNode, childFolder: FolderNode) => {
  ...
  return (
    <div className="flex items-stretch overflow-x-auto gap-0 border border-zinc-200 rounded-lg">
      {columns.map((folder, colIndex) => {
        const isActiveColumn = colIndex === columns.length - 1;
        const isEntering = colIndex === enterIndex;

        return (
          <div
            key={folder.id}
            className={`overflow-hidden transition-[max-width] duration-300 ease-out
              ${isEntering ? "max-w-0" : "max-w-[16rem]"}`}
          >
            <Column
              folder={folder}
...
interface ColumnProps {
  folder: FolderNode;
  selectedId?: string;
  onFolderClick: (parentFolder: FolderNode, childFolder: FolderNode) => void;
  isActiveColumn: boolean;
}

function Column({ folder, selectedId, onFolderClick, isActiveColumn }: ColumnProps) {
  return (
    <div
      className={`w-64 h-full shrink-0 border-l first:border-l-0
              ...
              return (
                <li key={node.id}>
                  <button
                    className={`w-full flex items-center justify-between px-3 py-2 text-sm text-left transition-colors
                      ${active ? "bg-blue-500 text-white" : "text-zinc-800 hover:bg-blue-50"}`}
                    onClick={() => onFolderClick(folder, node)}
                  >
                    <span className="flex items-center gap-2">
                      <span className={`${active ? "text-white" : "text-amber-500"}`}>📁</span>
                      {node.name}
